...
app.controller('AlimentoCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster) {
  ...
  //   $http.get('http://localhost:1345/grupo/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
...
// };
 ...
 $scope.cargaAlimentos = function(){
  ...
  //   $http.delete('http://localhost:1345/grupo/'+item.id , item)
  //   $scope.groups.splice($scope.groups.indexOf(item), 1);
  // };
...
$http.get('http://localhost:1345/alimento/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
  ...
  $scope.deleteAlimento = function(articulo){
    $http.delete('http://localhost:1345/alimento/'+alimento.id , alimento)
  ...
  $scope.doneEditingAlimento = function(item){
    ...
    if (MyService.data.idenAlimento){
      $scope.pop4();
      $http.put('http://localhost:1345/alimento/'+MyService.data.idenAlimento , alimentoAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://localhost:1345/alimento/', alimentoAct)
    }
    $scope.items = null;
